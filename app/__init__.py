...
def create_app(config_name: str = 'development') -> Flask:
    ...
    def metrics():
        ...
        try:
            ...
            # Application metrics
            app_metrics = []
            app_metrics.append(f'# HELP routeforce_cpu_usage CPU usage percentage')
            app_metrics.append(f'# TYPE routeforce_cpu_usage gauge')
            app_metrics.append(f'routeforce_cpu_usage {cpu_percent}')
            
            app_metrics.append(f'# HELP routeforce_memory_usage Memory usage percentage')
            app_metrics.append(f'# TYPE routeforce_memory_usage gauge')
            app_metrics.append(f'routeforce_memory_usage {memory.percent}')
            
            app_metrics.append(f'# HELP routeforce_disk_usage Disk usage percentage')
            app_metrics.append(f'# TYPE routeforce_disk_usage gauge')
            app_metrics.append(f'routeforce_disk_usage {disk.percent}')
            
            app_metrics.append(f'# HELP routeforce_uptime Application uptime in seconds')
            app_metrics.append(f'# TYPE routeforce_uptime counter')
            app_metrics.append(f'routeforce_uptime {time.time() - psutil.boot_time()}')
            
            return '\n'.join(app_metrics), 200, {'Content-Type': 'text/plain; charset=utf-8'}
