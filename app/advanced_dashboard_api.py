...
advanced_dashboard_bp = Blueprint('advanced_dashboard', __name__, url_prefix='/advanced')
...
def get_ml_insights():
    ...
    try:
        ...
        ensemble_status = {
            ...
            'last_training': 'Recently' if analytics.advanced_models_trained else 'Not trained'
        }
        ...
        return jsonify({
            ...
            'ml_insights': {
                ...
                'feature_importance': analytics.ensemble_engine.feature_importance if hasattr(analytics.ensemble_engine, 'feature_importance') else {}
            },
...
def get_performance_trends():
    ...
    try:
        ...
        for metric_type, data in trends_data.items():
            if len(data) >= 2:
                recent_avg = sum(item['value'] for item in data[-10:]) / min(len(data), 10)
                older_avg = sum(item['value'] for item in data[:10]) / min(len(data), 10)
                
                if metric_type == 'api_response_time' or metric_type == 'error_rate':
                    # Lower is better
                    trend = 'improving' if recent_avg < older_avg else 'degrading' if recent_avg > older_avg * 1.1 else 'stable'
                else:
                    # For CPU/memory, stable is better
                    trend = 'stable' if abs(recent_avg - older_avg) / older_avg < 0.1 else 'changing'
                
                trend_analysis[metric_type] = {
                    'trend': trend,
                    'recent_avg': round(recent_avg, 2),
                    'change_pct': round(((recent_avg - older_avg) / older_avg) * 100, 1) if older_avg > 0 else 0
                }
        ...
        for metric_name, metric_data in current_metrics.items():
            if isinstance(metric_data, dict) and 'value' in metric_data:
                ...
                if metric_name == 'cpu_usage':
                    forecast = min(current_value * 1.05, 100)  # Slight increase, capped at 100%
                elif metric_name == 'memory_usage':
                    forecast = min(current_value * 1.02, 16000)  # Slight increase, capped reasonably
        ...
        return jsonify({
            ...
            'real_time_alerts': {
                ...
                'alert_summary': {
                    ...
                    'high_priority': len([a for a in active_alerts if a['priority'] == 'high']),
                    'medium_priority': len([a for a in active_alerts if a['priority'] == 'medium']),
                    'low_priority': len([a for a in active_alerts if a['priority'] == 'low']),
