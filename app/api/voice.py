...
def voice_commit():
    ...
    try:
        ...
        return success_response(
            data=result, message="Voice commit processed successfully"
        )
        ...
        return error_response(message="Failed to process voice commit", status_code=500)
...
def generate_code_from_voice():
    ...
    try:
        ...
        return success_response(
            data=result, message="Code generated successfully from voice input"
        )
        ...
        return error_response(
            message="Failed to generate code from voice input", status_code=500
        )
...
def save_voice_note():
    ...
    try:
        ...
        return success_response(data=result, message="Voice note saved successfully")

    except BadRequest as e:
        return error_response(message=str(e), status_code=400)
    except Exception as e:
        current_app.logger.error(f"Voice note error: {str(e)}")
        return error_response(message="Failed to save voice note", status_code=500)
...
def upload_voice_audio():
    ...
    try:
        ...
        return success_response(data=result, message="Audio file uploaded successfully")

    except BadRequest as e:
        return error_response(message=str(e), status_code=400)
    except Exception as e:
        current_app.logger.error(f"Audio upload error: {str(e)}")
        return error_response(message="Failed to upload audio file", status_code=500)
...
def voice_settings():
    ...
    if request.method == "GET":
        ...
        return success_response(
            data=settings, message="Voice settings retrieved successfully"
        )
        ...
        try:
            ...
            return success_response(
                data=updated_settings, message="Voice settings updated successfully"
            )

        except Exception as e:
            current_app.logger.error(f"Settings update error: {str(e)}")
            return error_response(
                message="Failed to update voice settings", status_code=500
            )


def _generate_code(
    description: str, language: str, function_name: str = "", component_name: str = ""
) -> str:
    """
