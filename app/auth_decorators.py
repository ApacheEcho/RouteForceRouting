    ...
    def decorator(f: Callable) -> Callable:
        ...
        def decorated_function(*args, **kwargs):
            try:
                ...
                # Log access
                logger.info(f"Authenticated access: {user['email']} ({user['role']}) to {request.endpoint}")
        ...
        def decorated_function(*args, **kwargs):
            try:
                ...
                if user['role'] == 'driver':
                    ...
                    driver_id = request.view_args.get('driver_id') or request.args.get('driver_id')
    ...
    def decorator(f: Callable) -> Callable:
        ...
        def decorated_function(*args, **kwargs):
            try:
                ...
                # Log the action
                logger.info(f"AUDIT: User {user_email} performed {action} on {resource or 'unknown'}")
                
                # Execute the function
                result = f(*args, **kwargs)
                
                # Log successful completion
                logger.info(f"AUDIT: Action {action} completed successfully for user {user_email}")
                
                return result
                
            except Exception as e:
                # Log failed action
                logger.error(f"AUDIT: Action {action} failed for user {user_email}: {str(e)}")
                raise
