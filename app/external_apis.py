...
class WeatherIntegration:
    ...
    def _calculate_weather_impact(
        self, conditions: str, wind_speed: float, humidity: float
    ) -> float:
...
class ExternalDataManager:
    ...
    def optimize_route_with_traffic(
        ...
        try:
            ...
            optimized_waypoints = self.maps.optimize_waypoints(
        ...
        try:
            ...
            if enhanced_data.get("traffic_delay", 0) > 15:
                recommendations.append(
                    f"Heavy traffic detected. Consider departing {int(enhanced_data['traffic_delay'])} minutes earlier."
                )
            ...
            if weather_impact > 0.5:
                ...
                recommendations.append(
                    f"Poor weather conditions ({conditions}). Allow extra time and drive carefully."
                )

                if enhanced_data.get("precipitation", 0) > 2:
                    recommendations.append(
                        "Rain expected. Ensure vehicles have good tires and visibility equipment."
                    )
            ...
            if hour in [7, 8, 17, 18, 19]:
                recommendations.append(
                    "Rush hour traffic. Consider alternative departure times."
                )
