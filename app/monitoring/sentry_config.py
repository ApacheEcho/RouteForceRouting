...
def init_sentry(app: Flask = None):
    ...
    try:
        ...
        logging.info(f"Sentry initialized successfully for environment: {config.environment}")
        return True
...
def before_send_transaction_filter(event, hint):
    ...
    if any(skip in transaction_name.lower() for skip in ['health', 'static', 'favicon']):
        return None
...
class SentryHelper:
    ...
    @staticmethod
    def capture_performance_metrics(algorithm_type, execution_time, memory_usage, location_count):
        ...
        with sentry_sdk.push_scope() as scope:
            ...
            sentry_sdk.set_context("performance", {
                ...
                "location_count": location_count,
                "efficiency_score": location_count / execution_time if execution_time > 0 else 0
            })
            
            # Create custom metric
            sentry_sdk.capture_message(
                f"Route optimization completed: {algorithm_type}",
                level="info"
            )
