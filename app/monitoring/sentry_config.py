...
def init_sentry(app: Flask = None):
    ...
    try:
        ...
        sentry_sdk.init(
            ...
            integrations=[
                FlaskIntegration(
                    transaction_style="endpoint", record_sql_params=True
                ),
...
def before_send_transaction_filter(event, hint):
    ...
    if any(
        ...
        for skip in ["health", "static", "favicon"]
    ):
        return None
...
class SentryHelper:
    ...
    @staticmethod
    def capture_route_optimization_error(
        algorithm_type, location_count, error
    ):
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if (
                    execution_time > 1.0
                ):  # Functions taking longer than 1 second
