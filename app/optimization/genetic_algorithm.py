...
class GeneticAlgorithm:
    ...
    def optimize(self, stores: List[Dict[str, Any]], constraints: Dict[str, Any] = None) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
        """
        ...
        for generation in range(self.config.generations):
            ...
            # Log progress
            if generation % 50 == 0:
                logger.info(f"Generation {generation}: Best distance = {current_best.distance:.2f}km")
        
        # Convert best route back to store format
        optimized_route = [stores[i] for i in self.best_individual.route]
        
        # Calculate improvement
        improvement = ((initial_best.distance - self.best_individual.distance) / initial_best.distance) * 100
    ...
    def _fill_remaining(self, child_route: List[int], parent_route: List[int], start_pos: int):
