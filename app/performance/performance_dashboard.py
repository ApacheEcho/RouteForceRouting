...
class MetricsCollector:
    ...
    def collect_system_metrics(self) -> SystemMetrics:
        ...
        try:
            ...
            network_sent = (
                network.bytes_sent - self.last_network_counters.bytes_sent
            ) / time_delta
            network_recv = (
                network.bytes_recv - self.last_network_counters.bytes_recv
            ) / time_delta
    ...
    def get_historical_data(self, minutes: int = 60) -> Dict[str, Any]:
        ...
        recent_app = [
            m.to_dict() for m in self.app_history if m.timestamp > cutoff_time
        ]
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        # Calculate averages
        avg_cpu = sum(m.cpu_percent for m in recent_system) / len(
            recent_system
        )
        avg_memory = sum(m.memory_percent for m in recent_system) / len(
            recent_system
        )
        avg_disk = sum(m.disk_percent for m in recent_system) / len(
            recent_system
        )
        ...
        if recent_app:
            avg_response_time = sum(
                m.avg_response_time for m in recent_app
            ) / len(recent_app)
        ...
        warning_alerts = sum(
            1 for alert in self.alerts if alert.severity == "warning"
        )
...
def get_current_performance():
    ...
    return jsonify(
        performance_dashboard.metrics_collector.get_current_metrics()
    )
...
def get_historical_performance():
    ...
    return jsonify(
        performance_dashboard.metrics_collector.get_historical_data(minutes)
    )
...
def get_performance_summary():
    ...
    return jsonify(
        performance_dashboard.metrics_collector.get_performance_summary()
    )
