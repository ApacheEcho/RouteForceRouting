...
from app.auth_decorators import analytics_access_required, admin_required, dispatcher_required, audit_log
...
def get_overview_data():
    ...
    try:
        ...
        try:
            performance_metrics = database_service.get_performance_metrics(days_back=timeframe_days)
        except Exception as db_error:
            current_app.logger.warning(f"Database connection issue, using fallback data: {str(db_error)}")
        ...
        try:
            ...
            current_app.logger.warning(f"Database insights unavailable, using generated data: {str(db_error)}")
            recent_insights = []
        
        # Calculate key metrics
        key_metrics = {
            'total_routes': performance_metrics.get('total_routes', 0),
            'avg_fuel_efficiency': round(performance_metrics.get('avg_fuel_efficiency', 0), 1),
...
def get_active_routes():
    ...
    try:
        ...
        return jsonify({
            'success': False,
            'error': 'Failed to retrieve active routes'
        }), 500
...
def generate_enterprise_recommendations():
    ...
    try:
        ...
        performance_metrics = database_service.get_performance_metrics(days_back=timeframe)
...
def export_dashboard_data():
    ...
    try:
        ...
        if 'metrics' in data_types:
            export_data['performance_metrics'] = database_service.get_performance_metrics(timeframe)
        
        if 'insights' in data_types:
            export_data['insights'] = database_service.get_route_insights(days_back=timeframe)
        
        if 'routes' in data_types:
            export_data['historical_routes'] = database_service.get_historical_routes(days_back=timeframe)
        ...
        if export_format == 'json':
            ...
            return jsonify({
                ...
                'data_summary': {
                    'records_count': sum(len(v) if isinstance(v, list) else 1 for v in export_data.values()),
...
def generate_mock_active_routes() -> List[Dict[str, Any]]:
    ...
    driver_names = ['John Smith', 'Jane Doe', 'Mike Johnson', 'Sarah Wilson', 'David Brown']
...
def generate_contextual_recommendations(fleet_insights: Dict, performance_metrics: Dict, focus_area: str) -> List[Dict[str, Any]]:
