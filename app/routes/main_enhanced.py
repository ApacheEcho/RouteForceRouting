...
def generate_route():
    ...
    try:
        ...
        try:
            stores = file_service.process_stores_file(
                file
...
def generate_route_api():
    ...
    try:
        ...
        try:
            ...
            return (
                jsonify(
                    {"error": "Route generation failed", "details": str(e)}
                ),
                500,
            )
        ...
        response_data = {
            ...
            "optimization_score": (
                getattr(metrics, "optimization_score", None)
                if metrics
                else None
            ),
        }
        ...
        logger.info(
            f"Route generated successfully: {len(route) if route else 0} stops"
        )
...
def validate_playbook_constraints():
    ...
    try:
        ...
        try:
            ...
            validation_result = file_service.validate_playbook_file(
                playbook_file
            )
...
def react_app():
    ...
    try:
        ...
        if os.path.exists(build_path):
            ...
            logger.warning(
                "React build not found, redirecting to external app"
            )
...
def react_static(filename):
    ...
    try:
        ...
        static_dir = os.path.join(
            current_app.root_path, "..", "frontend", "dist"
        )
