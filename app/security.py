...
class AdaptiveRateLimiter:
    ...
    def analyze_user_pattern(self, user_id: str, endpoint: str, timestamp: float) -> Dict[str, Any]:
    ...
    def _calculate_risk_score(self, user_id: str, pattern: Dict[str, Any], timestamp: float) -> Dict[str, Any]:
        """Calculate user risk score based on behavior patterns"""
        score = 0
        
        # Check request frequency
        recent_requests = [req for req in pattern['requests'] if req > timestamp - 300]  # Last 5 minutes
        ...
        if unique_endpoints == 1 and len(pattern['requests']) > 20:  # Hitting same endpoint repeatedly
            score += 25
        
        # Check for rapid-fire requests
        if len(recent_requests) >= 2:
            gaps = [recent_requests[i] - recent_requests[i-1] for i in range(1, len(recent_requests))]
    ...
    def check_rate_limit(self, user_id: str, endpoint: str) -> tuple[bool, Dict[str, Any]]:
        ...
        recent_requests = [
            req for req in self.user_patterns[user_id]['requests'] 
            if req > window_start
        ]
...
class APIKeyManager:
    ...
    def rotate_key(self, client_id: str) -> tuple[str, str]:
        ...
        if old_key in self.key_usage:
            ...
            self.key_usage[old_key]['grace_period_end'] = time.time() + 86400  # 24 hours
    ...
    def cleanup_deprecated_keys(self):
        ...
        for api_key, key_info in self.key_usage.items():
            if (key_info.get('deprecated') and 
                current_time > key_info.get('grace_period_end', 0)):
