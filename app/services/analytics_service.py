...
class AnalyticsService:
    ...
    def track_route_optimization(self, route_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if route_record["optimization_time"]:
                self.performance_data["optimization_times"].append(
                    route_record["optimization_time"]
                )
    ...
    def get_mobile_analytics(self, timeframe: str = "24h") -> Dict[str, Any]:
        ...
        try:
            ...
            device_types = Counter(
                session["device_type"] for session in recent_sessions
            )
            app_versions = Counter(
                session["app_version"] for session in recent_sessions
            )
    ...
    def get_route_analytics(self, timeframe: str = "24h") -> Dict[str, Any]:
        ...
        try:
            ...
            recent_routes = [
                route
                for route in self.route_analytics
            ...
            return {
                ...
                "performance_metrics": {
                    "median_optimization_time": (
                        round(median(optimization_times), 3)
                        if optimization_times
                        else 0
                    ),
                    "max_optimization_time": (
                        max(optimization_times) if optimization_times else 0
                    ),
                    "min_optimization_time": (
                        min(optimization_times) if optimization_times else 0
                    ),
                },
            }
        ...
        try:
            ...
            return {
                ...
                "mobile_percentage": round(
                    (mobile_requests / len(recent_api_calls)) * 100, 2
                ),
    ...
    def get_system_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            health_score = self._calculate_health_score(
                recent_response_times, error_events
            )
            ...
            return {
                ...
                "performance": {
                    "avg_api_response_time": (
                        round(mean(recent_response_times), 3)
                        if recent_response_times
                        else 0
                    ),
                    "avg_optimization_time": (
                        round(mean(recent_optimization_times), 3)
                        if recent_optimization_times
                        else 0
                    ),
                },
