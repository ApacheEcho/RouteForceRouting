...
class RoutingService:
        ...
        try:
            ...
            if optimization_engine:
                optimization_engine.track_optimization_completion({
                    ...
                    'processing_time': processing_time,
                    'optimization_score': optimization_score,
                    'improvement_percent': algorithm_metrics.get('improvement_percent', 0),
                    'timestamp': time.time()
                })
    ...
    def _generate_route_genetic(self, stores: List[Dict], constraints: Dict, route_id: str = None) -> tuple[List[Dict], Dict]:
        ...
        try:
            ...
            optimized_route, optimization_metrics = genetic_optimizer.optimize(stores, constraints)
            
            if route_id:
                self._broadcast_optimization_progress(route_id, {
                    'stage': 'genetic_completed',
                    'generations': optimization_metrics.get('generations', 0),
                    'improvement': optimization_metrics.get('improvement_percent', 0)
                })
            ...
            return optimized_route, {
                ...
                'improvement_percent': optimization_metrics.get('improvement_percent', 0),
