...
def handle_route_progress(data):
    ...
    logger.info(
        f"Route {route_id} progress: {current_stop}/{active_routes[route_id]['total_stops']}"
    )
...
def broadcast_route_update(route_id: str, update_data: dict[str, Any]):
    ...
    socketio.emit(
        "route_update",
        {"route_id": route_id, "data": update_data, "timestamp": time.time()},
        room=room,
    )
        ...
        "rooms": {
            ...
            "route_tracking": len(
                [
                    ...
                    if any(r.startswith("route_") for r in c["rooms"])
                ]
            ),
        },
    }
...
def background_thread():
    ...
    def update_loop():
        ...
        while error_count < max_errors and not shutdown_event.is_set():
            try:
                ...
                stale_routes = [
                    route_id
                    for route_id, route_data in list(active_routes.items())
                    if current_time
                    - route_data.get("last_update", route_data["created_at"])
                    > 3600
                ]
    ...
    thread = threading.Thread(
        target=update_loop, daemon=True, name="RouteForceWebSocketBackground"
    )
    thread.start()
