...
from sklearn.ensemble import RandomForestRegressor, IsolationForest, GradientBoostingRegressor, VotingRegressor
...
class AdvancedAnalytics:
    ...
    def load_historical_data(self, days_back: int = 30) -> None:
        ...
        try:
            historical_routes = self.db_service.get_historical_routes(days_back=days_back)
            self.historical_data = historical_routes
            logger.info(f"Loaded {len(historical_routes)} historical routes from database")
    ...
    def get_insights_from_database(self, route_id: Optional[str] = None, 
                                 days_back: int = 30) -> List[Dict[str, Any]]:
        """Retrieve insights from database"""
        try:
            return self.db_service.get_route_insights(route_id=route_id, days_back=days_back)
        except Exception as e:
            logger.error(f"Failed to retrieve insights: {str(e)}")
            return []
    
    def get_performance_metrics_from_database(self, days_back: int = 30) -> Dict[str, Any]:
    ...
    def _enrich_route_data(self, route_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'distance' in route_data and 'duration' in route_data:
            enriched['speed_avg'] = route_data['distance'] / max(route_data['duration'], 1)
            
        if 'fuel_used' in route_data and 'distance' in route_data:
            enriched['fuel_efficiency'] = route_data['distance'] / max(route_data['fuel_used'], 0.1)
        ...
        if 'stops' in route_data:
            ...
            enriched['avg_stop_distance'] = route_data.get('distance', 0) / max(len(route_data['stops']), 1)
    ...
    def _train_models(self) -> None:
        ...
        feature_columns = [
            'distance', 'stops_count', 'hour_of_day', 'day_of_week',
            'is_weekend', 'is_rush_hour', 'avg_stop_distance'
        ]
    ...
    def _train_advanced_ensemble(self) -> None:
        ...
        if len(self.historical_data) < 200:
            logger.info("Insufficient data for advanced ensemble training (need 200+ samples)")
            return
        ...
        try:
            ...
            if all(col in df.columns for col in ['distance', 'stops_count']):
                df['distance_per_stop'] = df['distance'] / np.maximum(df['stops_count'], 1)
            ...
            training_results = self.ensemble_engine.train_ensemble_with_uncertainty(X_scaled, y)
            
            self.advanced_models_trained = True
            self.feature_columns = available_features
            
            logger.info(f"Advanced ensemble trained successfully with {len(training_results['trained_models'])} models")
            
        except Exception as e:
            logger.error(f"Failed to train advanced ensemble: {str(e)}")
    
    def predict_route_performance(self, route_features: Dict[str, Any]) -> PredictionResult:
        ...
        if not self.route_predictor or not hasattr(self, 'feature_columns'):
            ...
            return PredictionResult(
                ...
                predicted_duration=route_features.get('distance', 10) * 0.05,  # Default: 3 min per km
                predicted_fuel_cost=route_features.get('distance', 10) * 0.12,  # Default: $0.12 per km
                confidence_interval=(0.8, 1.2),
                risk_factors=['Model not trained - using default estimates'],
                optimization_suggestions=['Collect more route data for better predictions']
            )
        ...
        for col in self.feature_columns:
            if col in route_features:
                ...
                defaults = {
                    'distance': 10, 'stops_count': 5, 'hour_of_day': 10,
                    'day_of_week': 2, 'is_weekend': 0, 'is_rush_hour': 0,
                    'avg_stop_distance': 2
                }
    ...
    def analyze_route_efficiency(self, route_id: str, route_data: Dict[str, Any]) -> RouteInsight:
    ...
    def detect_performance_trends(self, timeframe_days: int = 30) -> List[PerformanceTrend]:
        ...
        if 'fuel_efficiency' in df.columns and len(df) > 5:
            ...
            change_pct = ((recent_efficiency - older_efficiency) / older_efficiency) * 100
    ...
    def get_fleet_insights(self) -> Dict[str, Any]:
        ...
        if len(self.historical_data) < 5:
            return {
                ...
                'insights': [],
                'trends': [],
                'recommendations': ['Collect more route data for comprehensive insights']
            }
...
class AdvancedMLModels:
    ...
    def feature_engineering_advanced(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'distance' in df.columns and 'stops_count' in df.columns:
            enhanced_df['distance_per_stop'] = df['distance'] / np.maximum(df['stops_count'], 1)
        ...
        if 'hour_of_day' in df.columns:
            ...
            enhanced_df['is_peak_morning'] = ((df['hour_of_day'] >= 7) & (df['hour_of_day'] <= 9)).astype(int)
            enhanced_df['is_peak_evening'] = ((df['hour_of_day'] >= 17) & (df['hour_of_day'] <= 19)).astype(int)
        ...
        if 'vehicle_type' in df.columns:
            ...
            enhanced_df['vehicle_efficiency'] = df['vehicle_type'].map(vehicle_efficiency).fillna(1.0)
        
        # Driver experience simulation
        if 'driver_id' in df.columns:
            driver_count = df['driver_id'].value_counts()
            enhanced_df['driver_experience'] = df['driver_id'].map(driver_count).fillna(1)
            enhanced_df['driver_experience_normalized'] = enhanced_df['driver_experience'] / enhanced_df['driver_experience'].max()
        
        return enhanced_df
    
    def train_advanced_models(self, historical_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        feature_columns = [
            'distance', 'stops_count', 'hour_of_day', 'day_of_week',
            'is_weekend', 'is_rush_hour', 'distance_per_stop', 'complexity_score',
            'hour_sin', 'hour_cos', 'is_peak_morning', 'is_peak_evening',
            'weather_impact', 'traffic_density', 'vehicle_efficiency',
        ...
        # Feature selection
        self.feature_selector = SelectKBest(score_func=f_regression, k=min(10, len(available_features)))
        ...
        cv_scores = cross_val_score(self.duration_model, X_scaled, y_duration, cv=5, scoring='neg_mean_squared_error')
        ...
        if 'fuel_used' in df_enhanced.columns:
            ...
            self.fuel_model = GradientBoostingRegressor(n_estimators=200, random_state=42)
        ...
        self.model_metadata = {
            ...
            'selected_features': self.feature_selector.get_feature_names_out(available_features).tolist(),
        ...
        logger.info(f"Advanced models trained successfully. CV Score: {-cv_scores.mean():.3f} ± {cv_scores.std():.3f}")
    ...
    def predict_with_uncertainty(self, route_features: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for col in available_features:
            if col in df_enhanced.columns:
                ...
                defaults = {
                    'distance': 10, 'stops_count': 5, 'hour_of_day': 10,
                    'day_of_week': 2, 'is_weekend': 0, 'is_rush_hour': 0,
                    'distance_per_stop': 2, 'complexity_score': 50,
                    'hour_sin': 0, 'hour_cos': 1, 'is_peak_morning': 0,
                    'is_peak_evening': 0, 'weather_impact': 1.0,
                    'traffic_density': 1.0, 'vehicle_efficiency': 1.0,
    ...
    def save_models(self, model_dir: str = 'models'):
        ...
        if self.duration_model:
            joblib.dump(self.duration_model, os.path.join(model_dir, 'duration_model.pkl'))
        if self.fuel_model:
    ...
    def load_models(self, model_dir: str = 'models'):
        ...
        try:
            self.duration_model = joblib.load(os.path.join(model_dir, 'duration_model.pkl'))
...
class AdvancedEnsembleEngine:
    ...
    def train_ensemble_with_uncertainty(self, X: np.ndarray, y: np.ndarray) -> Dict[str, Any]:
        ...
        for name, model in self.base_models.items():
            try:
                ...
                logger.info(f"Trained {name}: CV RMSE = {trained_models[name]['cv_rmse']:.3f}, CV R² = {trained_models[name]['cv_r2']:.3f}")
        ...
        if len(predictions) >= 3:
            ...
            logger.info(f"Ensemble model: RMSE = {ensemble_rmse:.3f}, R² = {ensemble_r2:.3f}")
    ...
    def _train_uncertainty_models(self, X: np.ndarray, y: np.ndarray, predictions: List[np.ndarray]):
    ...
    def predict_with_uncertainty(self, X: np.ndarray) -> Dict[str, Any]:
        ...
        if not self.base_models:
            raise ValueError("Models not trained. Call train_ensemble_with_uncertainty first.")
        ...
        return {
            ...
            'model_agreement': 1.0 - (epistemic_uncertainty / (np.abs(ensemble_pred) + 1e-8))
        }
    
    def explain_prediction(self, X: np.ndarray, feature_names: List[str]) -> Dict[str, Any]:
        ...
        for name, model_info in self.base_models.items():
            if 'model' in model_info:
                try:
                    pred = model_info['model'].predict(X)
                    explanation['model_contributions'][name] = pred[0] if len(pred) > 0 else 0
                except Exception:
                    continue
        ...
        if 'variance' in self.uncertainty_models:
            try:
                ...
                explanation['uncertainty_sources']['prediction_variance'] = pred_variance[0] if len(pred_variance) > 0 else 0
            except Exception:
                pass
