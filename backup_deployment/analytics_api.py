...
def get_fleet_insights():
    ...
    try:
        ...
        return jsonify({"success": True, "fleet_insights": insights})
    ...
    try:
        ...
        if vehicle_count > 5:
            recommendations.append(
                "Consider fleet coordination to avoid overlapping territories"
            )
        elif vehicle_count == 1:
            recommendations.append(
                "Single vehicle - optimize for sequential efficiency"
            )

        # Get insights from historical data
        fleet_insights = analytics.get_fleet_insights()
        if fleet_insights.get("recommendations") and isinstance(
            fleet_insights["recommendations"], list
        ):
                ...
                "prediction": {
                    ...
                    "uncertainty": {
                        ...
                        "epistemic_uncertainty": round(
                            prediction.uncertainty.epistemic_uncertainty, 2
                        ),
                        "aleatoric_uncertainty": round(
                            prediction.uncertainty.aleatoric_uncertainty, 2
                        ),
                    },
                    "risk_factors": prediction.risk_factors,
                    "optimization_suggestions": prediction.optimization_suggestions,
        ...
        return jsonify(
            {
                ...
                "monitoring": {
                    ...
                    "monitoring_status": (
                        "active" if monitor.monitoring_active else "inactive"
                    ),
                },
                "timestamp": datetime.now().isoformat(),
            }
