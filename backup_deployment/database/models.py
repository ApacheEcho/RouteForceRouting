...
class DatabaseManager:
    ...
    def __init__(self, database_url: str = "postgresql://user:password@localhost/routeforce"):
    ...
    def get_historical_routes(self, limit: int = 1000):
        ...
        session = self.get_session()
        try:
            routes = session.query(Route).order_by(Route.timestamp.desc()).limit(limit).all()
    ...
    def get_unacknowledged_alerts(self):
        ...
        session = self.get_session()
        try:
            alerts = session.query(AnalyticsAlert).filter(
                AnalyticsAlert.is_acknowledged == False
            ).order_by(AnalyticsAlert.created_at.desc()).all()
    ...
    def acknowledge_alert(self, alert_id: str, user_id: str):
        ...
        session = self.get_session()
        try:
            alert = session.query(AnalyticsAlert).filter(
