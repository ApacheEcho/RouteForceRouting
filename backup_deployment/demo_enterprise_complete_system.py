...
def demo_complete_enterprise_system():
    ...
    stores = [
        {"id": 1, "name": "Manhattan Hub", "lat": 40.7128, "lon": -74.0060, "priority": 1, "demand": 100},
        {"id": 2, "name": "Brooklyn Center", "lat": 40.6782, "lon": -73.9442, "priority": 1, "demand": 85},
        {"id": 3, "name": "Queens Plaza", "lat": 40.7282, "lon": -73.7949, "priority": 2, "demand": 70},
        {"id": 4, "name": "Bronx Station", "lat": 40.8176, "lon": -73.9782, "priority": 2, "demand": 60},
        {"id": 5, "name": "Staten Island", "lat": 40.5795, "lon": -74.1502, "priority": 3, "demand": 45},
        {"id": 6, "name": "Long Island City", "lat": 40.7505, "lon": -73.9934, "priority": 1, "demand": 90},
        {"id": 7, "name": "Jersey City", "lat": 40.7178, "lon": -74.0431, "priority": 2, "demand": 75},
        {"id": 8, "name": "Hoboken", "lat": 40.7439, "lon": -74.0324, "priority": 3, "demand": 55},
        {"id": 9, "name": "Newark", "lat": 40.7357, "lon": -74.1724, "priority": 2, "demand": 65},
        {"id": 10, "name": "Yonkers", "lat": 40.9312, "lon": -73.8988, "priority": 3, "demand": 40},
        {"id": 11, "name": "White Plains", "lat": 41.0340, "lon": -73.7629, "priority": 3, "demand": 35},
        {"id": 12, "name": "New Rochelle", "lat": 40.9115, "lon": -73.7843, "priority": 2, "demand": 50}
    ]
    
    print(f"📍 Enterprise Scenario: {len(stores)} distribution centers across NY Metro Area")
    print(f"📦 Total demand: {sum(s['demand'] for s in stores)} units")
    print(f"🎯 Priority levels: {len(set(s['priority'] for s in stores))} (1=High, 2=Medium, 3=Low)")
    print()
    ...
    for i, algorithm in enumerate(algorithms):
        ...
        try:
            ...
            if 'initial_distance' in metrics and 'final_distance' in metrics:
                improvement = ((metrics['initial_distance'] - metrics['final_distance']) / metrics['initial_distance']) * 100
            else:
                improvement = 0
            ...
            if algorithm['name'] == 'Genetic Algorithm':
                ...
                print(f"   🔄 Convergence: {metrics.get('convergence_generation', 'N/A')}")
                
            elif algorithm['name'] == 'Simulated Annealing':
                print(f"   🌡️  Final temperature: {metrics.get('final_temperature', 'N/A'):.4f}")
                ...
                print(f"   🏅 Objectives optimized: {len(metrics.get('objectives_optimized', []))}")
            
            # Show optimized route (first 5 stops)
            print(f"   🚛 Optimized route preview:")
            for j, store in enumerate(optimized_route[:5]):
                print(f"      {j+1}. {store['name']} (P{store['priority']}, D{store['demand']})")
    ...
    if successful_algorithms:
        ...
        avg_time = sum(r['processing_time'] for r in successful_algorithms) / len(successful_algorithms)
        print(f"⏱️  Average processing time: {avg_time:.2f} seconds")
        
        avg_improvement = sum(r['improvement'] for r in successful_algorithms) / len(successful_algorithms)
        print(f"📈 Average route improvement: {avg_improvement:.1f}%")
        
        best_algorithm = max(successful_algorithms, key=lambda x: x['improvement'])
        print(f"🏆 Best performer: {best_algorithm['algorithm']} ({best_algorithm['improvement']:.1f}% improvement)")
