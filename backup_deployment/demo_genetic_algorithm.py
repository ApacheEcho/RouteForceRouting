...
def test_algorithm_comparison():
    ...
    print(
        f"  - Optimization Score: {default_data['metadata']['optimization_score']:.2f}"
    )
    ...
    genetic_response = requests.post(
        f"{BASE_URL}/api/v1/routes",
        json={
            "stores": DEMO_STORES,
            "constraints": {},
    ...
    print(
        f"  - Optimization Score: {genetic_data['metadata']['optimization_score']:.2f}"
    )
    ...
    print(f"\n📊 Performance Summary:")
    print(
        f"  - Speed Advantage: Default algorithm is {genetic_time/default_time:.1f}x faster"
    )
    print(
        f"  - Optimization Advantage: Genetic algorithm improved route by {ga_metrics['improvement_percent']:.1f}%"
    )
    print(
        f"  - Distance Saved: {ga_metrics['initial_distance'] - ga_metrics['final_distance']:.2f}km"
    )
...
def test_parameter_tuning():
    ...
    for config in configs:
        ...
        response = requests.post(
            ...
            json={
                "stores": DEMO_STORES,
                "constraints": {},
                "options": {"algorithm": "genetic", **config["params"]},
            },
        )
    ...
    print(
        f"  - Best Quality: {best_quality['name']} ({best_quality['distance']:.2f}km)"
    )
    print(f"  - Fastest: {fastest['name']} ({fastest['time']:.3f}s)")
    print(
        f"  - Best Improvement: {best_improvement['name']} ({best_improvement['improvement']:.1f}%)"
    )
...
def test_dedicated_endpoint():
    ...
    response = requests.post(
        f"{BASE_URL}/api/v1/routes/optimize/genetic",
        json={
            "stores": DEMO_STORES,
            "constraints": {},
            "genetic_config": {
...
def display_route_visualization(route_data):
    ...
    for i, store in enumerate(route):
        print(
            f"  {i+1:2d}. {store['name']} ({store['latitude']:.4f}, {store['longitude']:.4f})"
        )
