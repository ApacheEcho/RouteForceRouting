...
def test_api_integration():
    ...
    try:
        response = requests.post(
            "http://localhost:5002/api/v1/routes", json=request_data
        )
        if response.status_code == 201:
            data = response.json()
            print(f"✅ Main API endpoint successful")
            print(
                f"   Algorithm used: {data['metadata'].get('algorithm_used', 'unknown')}"
            )
            print(
                f"   Processing time: {data['metadata'].get('processing_time', 0):.3f}s"
            )
            print(
                f"   Optimization score: {data['metadata'].get('optimization_score', 0):.1f}"
            )
    ...
    try:
        response = requests.post(
            "http://localhost:5002/api/v1/routes/optimize/simulated_annealing",
            json=sa_request_data,
        )
        if response.status_code == 201:
            data = response.json()
...
def test_routing_service_integration():
    ...
    if metrics:
        ...
        if metrics.algorithm_metrics:
            ...
            print(
                f"Algorithm metrics keys: {list(metrics.algorithm_metrics.keys()) if isinstance(metrics.algorithm_metrics, dict) else 'Not a dict'}"
            )
