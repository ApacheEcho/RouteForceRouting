...
class RouteForceSystemTest:
    ...
    def test_frontend_accessibility(self):
        ...
        try:
            ...
            if response.status_code == 200:
                self.log_test("Frontend Accessibility", True, "Frontend is serving content")
                return True
            else:
        ...
        for endpoint in endpoints:
            try:
                ...
                if response.status_code in [200, 201]:
                    self.log_test(f"API {endpoint}", True, f"HTTP {response.status_code}")
                    success_count += 1
                else:
    ...
    def test_ml_integration(self):
        ...
        try:
            response = requests.get(f"{self.backend_url}/advanced/api/ml-insights", timeout=10)
            if response.status_code == 200:
                data = response.json()
                if data.get('success'):
                    self.log_test("ML Integration", True, "ML insights retrieved successfully")
                    return True
                else:
                    self.log_test("ML Integration", False, f"ML error: {data.get('error', 'unknown')}")
                    return False
            else:
    ...
    def test_performance_monitoring(self):
        ...
        try:
            response = requests.get(f"{self.backend_url}/advanced/api/performance-trends", timeout=5)
            if response.status_code == 200:
                data = response.json()
                if data.get('success'):
                    self.log_test("Performance Monitoring", True, "Performance data available")
                    return True
                else:
                    self.log_test("Performance Monitoring", False, "No performance data")
                    return False
            else:
                self.log_test("Performance Monitoring", False, f"HTTP {response.status_code}")
                return False
        except Exception as e:
        ...
        for algorithm in algorithms:
            try:
                ...
                if response.status_code == 200:
                    data = response.json()
                    if data.get('success'):
                        self.log_test(f"Algorithm {algorithm}", True, 
                                    f"Optimized route with {len(data.get('optimized_route', []))} stops")
                        success_count += 1
                    else:
                        self.log_test(f"Algorithm {algorithm}", False, 
                                    f"Optimization failed: {data.get('error', 'unknown')}")
                else:
                    self.log_test(f"Algorithm {algorithm}", False, f"HTTP {response.status_code}")
            except Exception as e:
    ...
    def run_full_test_suite(self):
        ...
        if passed_tests == total_tests:
            print("✅ ALL SYSTEMS OPERATIONAL - Enterprise RouteForce is ready for production!")
        elif passed_tests >= total_tests * 0.8:
            print("⚠️  MOSTLY OPERATIONAL - Minor issues detected, but core functionality is working")
        else:
            print("❌ CRITICAL ISSUES - System requires attention before production deployment")
