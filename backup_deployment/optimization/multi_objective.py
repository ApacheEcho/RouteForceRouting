...
class MultiObjectiveOptimizer:
    ...
    def __init__(self, config: MultiObjectiveConfig):
        ...
        logger.info(f"Initialized Multi-Objective optimizer with objectives: {config.objectives}")
    
    def optimize(self, stores: List[Dict[str, Any]], constraints: Optional[Dict[str, Any]] = None) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
        """
        ...
        try:
            ...
            for generation in range(self.config.generations):
                ...
                new_population = self._evolutionary_operators(population, fronts, crowding_distances)
                population = new_population
                
                # Log progress
                if generation % 20 == 0:
                    logger.debug(f"Generation {generation}: Pareto front size = {len(fronts[0])}")
            ...
            best_compromise_idx = self._select_best_compromise(pareto_front, final_objectives)
            ...
            logger.info(f"Multi-objective optimization completed in {processing_time:.4f}s")
    ...
    def _non_dominated_sort(self, objective_values: List[List[float]]) -> List[List[int]]:
    ...
    def _calculate_crowding_distance(self, fronts: List[List[int]], objective_values: List[List[float]]) -> List[float]:
        ...
        for front in fronts:
            ...
            for obj_idx in range(len(self.config.objectives)):
                ...
                # Calculate range
                obj_range = objective_values[front_sorted[-1]][obj_idx] - objective_values[front_sorted[0]][obj_idx]
                if obj_range == 0:
                    continue
    ...
    def _evolutionary_operators(self, population: List[List[int]], fronts: List[List[int]], crowding_distances: List[float]) -> List[List[int]]:
        ...
        while len(new_population) < self.config.population_size:
            ...
            parent1_idx = self._tournament_selection(population, fronts, crowding_distances)
            parent2_idx = self._tournament_selection(population, fronts, crowding_distances)
    ...
    def _select_elite(self, fronts: List[List[int]], crowding_distances: List[float], elite_size: int) -> List[int]:
    ...
    def _tournament_selection(self, population: List[List[int]], fronts: List[List[int]], crowding_distances: List[float]) -> int:
        """Tournament selection based on dominance and crowding distance"""
        candidates = random.sample(range(len(population)), min(self.config.tournament_size, len(population)))
        ...
        for candidate in candidates[1:]:
            ...
            if (candidate_front < best_front or 
                (candidate_front == best_front and crowding_distances[candidate] > crowding_distances[best_candidate])):
    ...
    def _select_best_compromise(self, pareto_front: List[int], objective_values: List[List[float]]) -> int:
        ...
        for idx in pareto_front:
            distance = sum((objective_values[idx][i] - ideal_point[i]) ** 2 for i in range(len(ideal_point)))
    ...
    def _calculate_metrics(self, pareto_front: List[int], objective_values: List[List[float]], processing_time: float, best_compromise_idx: int) -> Dict[str, Any]:
        ...
        if len(pareto_front) > 0:
            try:
                metrics['hypervolume'] = self._calculate_hypervolume(pareto_front, objective_values)
            except:
                metrics['hypervolume'] = 0.0
        
        return metrics
    
    def _calculate_hypervolume(self, pareto_front: List[int], objective_values: List[List[float]]) -> float:
    ...
    def _create_distance_matrix(self, stores: List[Dict[str, Any]]) -> List[List[float]]:
    ...
    def _calculate_distance(self, store1: Dict[str, Any], store2: Dict[str, Any]) -> float:
    ...
    def _calculate_time_window_violations(self, route: List[int]) -> float:
        ...
        for store_idx in route:
            ...
            if store_idx > 0:
                ...
                travel_time = self.distance_matrix[prev_store_idx][store_idx] / 50  # 50 km/h
