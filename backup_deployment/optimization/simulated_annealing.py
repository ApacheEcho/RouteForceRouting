...
class SimulatedAnnealingOptimizer:
    ...
    def __init__(self, config: SimulatedAnnealingConfig):
        ...
        self.cooling_function = self.cooling_schedules.get(
            ...
            self._exponential_cooling
        )
        
        self.neighborhood_function = self.neighborhood_operators.get(
            config.neighborhood_operator,
            self._swap_two_nodes
        )
        
        logger.info(f"Initialized SA optimizer with {config.cooling_schedule} cooling and {config.neighborhood_operator} neighborhood")
        ...
        try:
            ...
            while (temperature > self.config.final_temperature and 
                ...
                if temperature_reductions % 10 == 0:
                    logger.debug(f"Temperature: {temperature:.4f}, Best distance: {best_distance:.4f}")
            ...
            improvement_percent = ((self.metrics.initial_distance - best_distance) / self.metrics.initial_distance) * 100
            ...
            self.metrics.acceptance_rate = accepted_moves / total_moves if total_moves > 0 else 0
            ...
            logger.info(f"Accepted moves: {accepted_moves}/{total_moves} ({self.metrics.acceptance_rate:.1%})")
    ...
    def _linear_cooling(self, temperature: float, iteration: int) -> float:
        ...
        return self.config.initial_temperature - (iteration * (self.config.initial_temperature - self.config.final_temperature) / self.config.max_iterations)
