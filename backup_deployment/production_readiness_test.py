        ...
        for route_test in test_routes:
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.log_test(route_test["name"], "PASS", "Rate limited (good security)")
                else:
                    self.log_test(route_test["name"], "FAIL", f"Status: {response.status_code}")
    ...
    def test_security_features(self):
        ...
        try:
            ...
            for i in range(5):
                response = requests.post(
                    f"{self.backend_url}/api/optimize",
                    json={"stops": [{"lat": 37.7749, "lon": -122.4194, "name": "Test"}]},
                    timeout=5
                )
            ...
            if 429 in responses:
                self.log_test("Rate Limiting", "PASS", "Successfully blocks rapid requests")
            else:
        ...
        try:
            ...
            if response.status_code in [400, 422, 429]:
                self.log_test("Error Handling", "PASS", f"Proper error response: {response.status_code}")
            else:
                self.log_test("Error Handling", "FAIL", f"Unexpected status: {response.status_code}")
        except Exception as e:
