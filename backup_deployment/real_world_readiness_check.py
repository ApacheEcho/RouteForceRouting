    ...
    def assess_security(self):
        ...
        try:
            ...
            if response.status_code in [200, 401]:  # 401 means auth is working
                ...
                self.critical_issues.append("Authentication system not responding properly")
        except:
    ...
    def assess_performance(self):
        ...
        try:
            ...
            if response_time < 0.1:  # Under 100ms
                print(f"   ✅ API response time: {response_time*1000:.1f}ms (Excellent)")
                score += 25
            elif response_time < 0.5:  # Under 500ms
                print(f"   ✅ API response time: {response_time*1000:.1f}ms (Good)")
                score += 20
            else:
                print(f"   ⚠️  API response time: {response_time*1000:.1f}ms (Needs optimization)")
        ...
        try:
            test_data = {
                "stops": [
                    {"id": str(i), "lat": 37.7749 + i*0.01, "lng": -122.4194 + i*0.01, "name": f"Stop {i}"}
            ...
            if opt_time < 5:  # Under 5 seconds for 10 stops
                print(f"   ✅ Route optimization: {opt_time:.2f}s for 10 stops (Excellent)")
                score += 25
