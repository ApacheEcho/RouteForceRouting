...
"""
from flask import Blueprint, render_template, request, jsonify, Response, current_app, send_file, redirect, url_for
        ...
        if "GET" in rule.methods and len(rule.arguments) == 0:
            try:
                ...
                routes.append({
                    ...
                    'description': getattr(current_app.view_functions.get(rule.endpoint), '__doc__', 'No description')
                })
            except Exception:
...
def generate_route():
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Route generated successfully with {len(route_result) if route_result else 0} stops")
            ...
            return jsonify({
                ...
                'filters_applied': {
                    'proximity': proximity,
                    'time_window': f"{time_start} - {time_end}" if time_start and time_end else None,
                    'priority_only': priority_only,
                    'exclude_days': exclude_days,
                    'max_stores_per_chain': max_stores_per_chain,
                    'min_sales_threshold': min_sales_threshold
...
def generate_route_api():
    ...
    try:
        ...
        response_data = {
            ...
            'processing_time': getattr(metrics, 'processing_time', None) if metrics else None,
            'optimization_score': getattr(metrics, 'optimization_score', None) if metrics else None
        }
...
def validate_playbook_constraints():
    ...
    try:
        ...
        try:
            ...
            return jsonify(validation_result), 200 if validation_result.get('valid') else 400
            
        except Exception as e:
            logger.error(f"Playbook validation failed: {e}")
            return jsonify({'valid': False, 'error': 'Validation error', 'details': str(e)}), 400
...
def react_app():
    ...
    try:
        ...
        import os
        build_path = os.path.join(current_app.root_path, '..', 'frontend', 'dist', 'index.html')
