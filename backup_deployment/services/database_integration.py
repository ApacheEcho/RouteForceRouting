...
from app.database.models import (
    Route, RouteInsightDB, RoutePredictionDB, PerformanceTrendDB, Base
)
...
class DatabaseIntegrationService:
    ...
    def store_route_data(self, route_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            route = Route(
                route_id=route_data.get('route_id', f"route_{datetime.now().timestamp()}"),
    ...
    def get_historical_routes(self, 
    ...
    def get_route_insights(self, route_id: Optional[str] = None,
    ...
    def get_performance_metrics(self, days_back: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            route_metrics = db.session.query(
            ...
            ).filter(Route.timestamp >= cutoff_date).first()
            ...
            time_patterns = db.session.query(
             ...
             .order_by(Route.hour_of_day).all()
    ...
    def cleanup_old_data(self, days_to_keep: int = 90) -> Dict[str, int]:
        ...
        try:
            ...
            routes_to_delete = db.session.query(Route).filter(Route.timestamp < cutoff_date).count()
            insights_to_delete = db.session.query(RouteInsightDB).filter(RouteInsightDB.timestamp < cutoff_date).count()
            predictions_to_delete = db.session.query(RoutePredictionDB).filter(RoutePredictionDB.timestamp < cutoff_date).count()
            
            # Delete old records
            db.session.query(RouteInsightDB).filter(RouteInsightDB.timestamp < cutoff_date).delete()
            db.session.query(RoutePredictionDB).filter(RoutePredictionDB.timestamp < cutoff_date).delete()
