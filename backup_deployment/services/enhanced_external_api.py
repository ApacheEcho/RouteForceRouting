...
from app.external_apis import GoogleMapsIntegration, WeatherIntegration, TrafficInfo, WeatherInfo
...
class EnhancedExternalAPIService:
    ...
    def get_enhanced_route_data(self, origin: str, destination: str, waypoints: List[str] = None) -> ContextualRouteData:
        """
        ...
        try:
            ...
            delay_probability = self._calculate_delay_probability(traffic_info, weather_data)
    ...
    def get_real_time_updates(self, route_id: str, current_location: Tuple[float, float]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if traffic_data and traffic_data.get('severity', 'low') in ['high', 'severe']:
    ...
    def optimize_route_with_context(self, route_request: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # Standard route
            standard_route = self.get_enhanced_route_data(origin, destination, waypoints)
            route_options.append(('standard', standard_route))
            
            # Alternative routes if available
            try:
                alt_routes = self.maps_api.get_alternative_routes(origin, destination, waypoints)
                for i, alt_route in enumerate(alt_routes[:2]):  # Max 2 alternatives
                    enhanced_alt = self.get_enhanced_route_data(
                        alt_route['origin'], alt_route['destination'], alt_route.get('waypoints')
                    )
    ...
    def _calculate_fuel_impact(self, weather_data: List[WeatherInfo], traffic_info: TrafficInfo) -> float:
    ...
    def _calculate_delay_probability(self, traffic_info: TrafficInfo, weather_data: List[WeatherInfo]) -> float:
    ...
    def _calculate_risk_score(self, weather_data: List[WeatherInfo], traffic_info: TrafficInfo) -> float:
    ...
    def _identify_optimization_opportunities(self, route_data: Dict, traffic_info: TrafficInfo, weather_data: List[WeatherInfo]) -> List[str]:
        ...
        try:
            ...
            if 7 <= current_hour <= 9 or 17 <= current_hour <= 19:
                opportunities.append("Consider scheduling outside rush hours to avoid traffic")
            
            # Weather-based opportunities
            for weather in weather_data:
                if weather.precipitation > 2:
                    opportunities.append("Rain detected - allow extra time and reduce speed")
                if weather.wind_speed > 30:
                    opportunities.append("Strong winds - fuel efficiency may be reduced")
                if weather.visibility < 5:  # km
                    opportunities.append("Poor visibility - consider delaying departure")
    ...
    def _score_route_option(self, route_data: ContextualRouteData, preferences: Dict[str, Any]) -> float:
        ...
        try:
            ...
            if priority == 'speed':
                ...
                base_score -= route_data.fuel_impact_factor * 30  # Emphasize fuel efficiency
    ...
    def _generate_route_summary(self, route_data: ContextualRouteData) -> Dict[str, Any]:
        ...
        return {
            ...
            'overall_risk': 'low' if route_data.risk_score < 0.3 else 'medium' if route_data.risk_score < 0.7 else 'high',
