...
class TrafficService:
    ...
    def __init__(self, config: TrafficConfig = None):
        ...
        if not self.api_key:
            logger.warning("Google Maps API key not found. Traffic features will be limited.")
    ...
    def get_traffic_optimized_route(
        ...
        try:
            ...
            if len(waypoints) > self.config.max_waypoints:
                logger.warning(f"Too many waypoints ({len(waypoints)}), using clustering")
                waypoints = self._cluster_waypoints(waypoints, self.config.max_waypoints)
            ...
            optimized_route = self._build_optimized_route(stores, route_data, traffic_info)
    ...
    def get_traffic_data_for_segment(
        ...
        try:
            ...
            traffic_data = TrafficData(
                ...
                traffic_delay=leg.get('duration_in_traffic', {}).get('value', leg['duration']['value']) - leg['duration']['value'],
            ...
            for hours in future_hours:
                ...
                if route_data:
                    ...
                    predictions[f"{hours}h"] = {
                        ...
                        'total_duration': traffic_info.get('total_duration', 0),
                        'traffic_delay': traffic_info.get('total_traffic_delay', 0),
                        'recommended': traffic_info.get('total_traffic_delay', 0) < 300  # Less than 5 min delay
                    }
            
            return {
                'success': True,
                'predictions': predictions,
                'best_time': min(predictions.items(), key=lambda x: x[1]['total_duration'])[0] if predictions else None
            }
    ...
    def _get_google_maps_route(
        ...
        try:
            ...
            params = {
                ...
                'units': 'metric',
                'departure_time': constraints.get('departure_time', self.config.departure_time),
                'traffic_model': constraints.get('traffic_model', self.config.traffic_model)
            }
    ...
    def _process_traffic_data(self, route_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for leg in route['legs']:
                ...
                leg_traffic_duration = leg.get('duration_in_traffic', {}).get('value', leg_duration)
                ...
                segments.append({
                    ...
                    'traffic_condition': self._classify_traffic_condition(leg_duration, leg_traffic_duration)
                })
            ...
            return {
                ...
                'average_speed': (total_distance / total_duration * 3.6) if total_duration > 0 else 0,  # km/h
                'traffic_severity': self._classify_overall_traffic(total_duration, total_traffic_delay),
                'segments': segments,
    ...
    def _classify_traffic_condition(self, normal_duration: int, traffic_duration: int) -> str:
    ...
    def _fallback_route(self, stores: List[Dict[str, Any]], reason: str) -> Dict[str, Any]:
    ...
    def _is_cache_valid(self, cache_key: str) -> bool:
        ...
        return (time.time() - self.cache_timestamps[cache_key]) < self.config.cache_duration
