...
class AdvancedAnalyticsTest:
    ...
    def test_advanced_analytics_api(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"  Prediction confidence: {result.get('model_confidence', 'N/A')}"
                )
                logger.info(
                    f"  Uncertainty range: {result.get('confidence_interval', 'N/A')}"
                )
                return True
            else:
                logger.error(
                    f"✗ Advanced prediction API failed: {response.status_code}"
                )
                return False
    ...
    def test_advanced_dashboard_api(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"  Model accuracy: {insights.get('model_performance', {}).get('accuracy', 'N/A')}"
                )

                # Test predictive analytics
                response = self.session.get(
                    f"{self.base_url}/api/v1/dashboard/predictive-analytics"
                )
                if response.status_code == 200:
                    analytics = response.json()
                    logger.info("✓ Predictive analytics API successful")
                    logger.info(
                        f"  Expected routes today: {analytics.get('route_predictions', {}).get('expected_routes_today', 'N/A')}"
                    )

                    # Test optimization insights
                    response = self.session.get(
                        f"{self.base_url}/api/v1/dashboard/optimization-insights"
                    )
                    if response.status_code == 200:
                        optimization = response.json()
                        logger.info("✓ Optimization insights API successful")
                        logger.info(
                            f"  Best algorithm: {optimization.get('optimization_patterns', {}).get('best_performing_algorithm', 'N/A')}"
                        )
                        return True
    ...
    def test_ensemble_training(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"  Models trained: {len(result.get('trained_models', {}))}"
                )
                logger.info(
                    f"  Training samples: {result.get('metadata', {}).get('training_samples', 'N/A')}"
                )
                return True
            else:
    ...
    def test_uncertainty_quantification(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"  Epistemic uncertainty: {result.get('epistemic_uncertainty', 'N/A')}"
                )
                logger.info(
                    f"  Aleatoric uncertainty: {result.get('aleatoric_uncertainty', 'N/A')}"
                )
                logger.info(
                    f"  Model agreement: {result.get('model_agreement', 'N/A')}"
                )
                ...
                logger.error(
                    f"✗ Uncertainty quantification failed: {response.status_code}"
                )
                return False
    ...
    def test_real_time_alerts(self) -> bool:
        ...
        try:
            response = self.session.get(
                f"{self.base_url}/api/v1/dashboard/real-time-alerts"
            )
            if response.status_code == 200:
                ...
                logger.info(
                    f"  Alert types: {', '.join(alert_types) if alert_types else 'None'}"
                )
                return True
            else:
