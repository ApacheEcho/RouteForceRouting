...
def is_valid_visit_hours(visit_hours):
    if not isinstance(visit_hours, dict) or "start" not in visit_hours or "end" not in visit_hours:
        return False
    try:
...
def test_generate_route_with_invalid_file_type(client):
    import io
    bad_data = {
        'file': (io.BytesIO(b"not,a,csv"), 'not_a_csv.txt')
    }
    response = client.post('/generate', data=bad_data, content_type='multipart/form-data')
...
def test_generate_route_with_empty_file(client):
    ...
    response = client.post('/generate', data=empty_data, content_type='multipart/form-data')
    assert response.status_code == 400
...
@pytest.mark.parametrize("stores, playbook, expected_names", [
    ([{"name": "S1", "chain": "A"}], {"A": {"priority": 1}}, ["S1"]),
    ([{"name": "S1", "chain": "A"}, {"name": "S2", "chain": "B"}], {
        "A": {"priority": 2}, "B": {"priority": 1}}, ["S1", "S2"]),
])
...
# Edge cases for generate_route
@pytest.mark.parametrize("stores,playbook,expected", [
    ([], {}, []),  # Empty stores list
    ([{"name": "Store A", "chain": "Chain A"}], {"Chain A": {"priority": 1}}, [{"name": "Store A", "chain": "Chain A"}]),  # Single store
    ([{"name": "Store A", "chain": "Chain A"}], {"Chain A": {"visit_hours": {"start": "invalid", "end": "invalid"}}}, []),  # Malformed playbook
])
...
def test_disallowed_time_window():
    stores = [{"name": "Store A", "chain": "Chain A"}]
    playbook = {"Chain A": {"time_window": {"start": "00:00", "end": "01:00"}}}  # Will fail most times
