...
def test_enhanced_dashboard():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for result in data['results']:
                ...
                print(f"   {status} {result['algorithm']}: {improvement:.1f}% improvement, {processing_time:.3f}s")
        else:
    ...
    for algorithm in algorithms:
        try:
            response = requests.get(f"{base_url}/dashboard/api/algorithm/details/{algorithm}")
            if response.status_code == 200:
                data = response.json()
                algo_info = data['algorithm']
                print(f"✅ {algorithm} details: {algo_info['name']}")
                print(f"   Best for: {algo_info['best_for']}")
                print(f"   Avg improvement: {algo_info['performance']['avg_improvement']}")
            else:
