# Import RoutingService and MLRoutePredictor using importlib from their file paths
...
routing_service_path = os.path.join(project_root, "app/services/routing_service.py")
ml_predictor_path = os.path.join(project_root, "app/optimization/ml_predictor.py")

routing_service_mod = import_from_path("routing_service", routing_service_path)
ml_predictor_mod = import_from_path("ml_predictor", ml_predictor_path)

RoutingService = routing_service_mod.RoutingService
MLConfig = ml_predictor_mod.MLConfig
MLRoutePredictor = ml_predictor_mod.MLRoutePredictor
...
def test_ml_routing_service():
    ...
    try:
        ...
        print(
            f"   ✓ ML predictor available: {routing_service.ml_predictor is not None}"
        )
        ...
        prediction_result = routing_service.predict_route_performance(
            test_stores, context
        )
        ...
        print("\n4. Testing algorithm recommendation...")
        recommendation_result = routing_service.recommend_algorithm(
            test_stores, context
        )
        ...
        if ml_route_result.get("success"):
            print(
                f"     - Algorithm used: {ml_route_result['ml_recommendation']['recommended_algorithm']}"
            )
            print(
                f"     - Confidence: {ml_route_result['ml_recommendation']['confidence']:.2f}"
            )
            print(
                f"     - Reasoning: {ml_route_result['ml_recommendation']['reasoning']}"
            )
...
def test_heuristic_recommendations():
    ...
    test_cases = [
        ...
        {
            "name": "Large route",
            "stores": [{"id": "1", "lat": 40.7, "lon": -74.0}] * 20,
        },
    ]
    ...
    try:
        ...
        for i, test_case in enumerate(test_cases, 1):
            ...
            if recommendation.get("success"):
                print(
                    f"   ✓ Recommended algorithm: {recommendation['recommendation']['recommended_algorithm']}"
                )
                print(
                    f"   ✓ Confidence: {recommendation['recommendation']['confidence']}"
                )
                print(
                    f"   ✓ Reasoning: {recommendation['recommendation']['reasoning']}"
                )
            else:
