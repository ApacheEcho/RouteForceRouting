...
def create_test_stores(num_stores: int = 10) -> List[Dict[str, Any]]:
    ...
    for i in range(num_stores):
        stores.append({
            ...
            'latitude': round(37.7749 + random.uniform(-0.1, 0.1), 6),  # San Francisco area
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if 'simulated_annealing' in algorithms:
                ...
                print(f"   Description: {sa_config.get('description', 'No description')}")
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            if 'route' in route_data and 'metadata' in route_data:
                ...
                print(f"   Optimization score: {metadata.get('optimization_score', 'N/A'):.2f}")
...
def test_dedicated_simulated_annealing_endpoint():
    ...
    try:
        ...
        start_time = time.time()
        response = requests.post(f"{API_BASE}/routes/optimize/simulated_annealing", json=request_data)
        api_time = time.time() - start_time
...
def test_simulated_annealing_performance():
    ...
    for test_config in configurations:
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                print(f"  ✅ {test_config['name']}: {api_time:.2f}s API, {result['processing_time']:.2f}s processing")
                print(f"     Score: {result['optimization_score']:.2f}")
                
            else:
                print(f"  ❌ {test_config['name']}: Failed with status {response.status_code}")
    ...
    if results:
        ...
        for result in results:
            print(f"{result['name']:9} | {result['api_time']:8.2f} | {result['processing_time']:10.2f} | {result['optimization_score']:6.2f} | {result['route_length']:12}")
...
def test_error_handling():
    ...
    test_cases = [
        ...
        },
        {
            'name': 'Single store',
            'data': {
                "stores": [{"name": "Store1", "latitude": 37.7749, "longitude": -122.4194}],
                "constraints": {},
                "sa_config": {}
            },
    ...
    for test_case in test_cases:
        ...
        try:
            response = requests.post(f"{API_BASE}/routes/optimize/simulated_annealing", json=test_case['data'])
...
def main():
    ...
    test_results.append(("Route Creation with SA", test_create_route_with_simulated_annealing()))
    test_results.append(("Dedicated SA Endpoint", test_dedicated_simulated_annealing_endpoint()))
