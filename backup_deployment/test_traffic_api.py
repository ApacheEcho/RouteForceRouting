...
def test_traffic_optimization():
    ...
    test_data = {
        "stores": [
            {"id": "store_1", "address": "1600 Amphitheatre Parkway, Mountain View, CA"},
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if result.get('traffic_info'):
                print(f"Traffic conditions: {result['traffic_info'].get('conditions', 'N/A')}")
        else:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for i, alt in enumerate(result.get('alternatives', [])[:3]):  # Show first 3
                print(f"  Route {i+1}: {alt.get('summary', 'N/A')} - {alt.get('duration', 'N/A')}")
        else:
            print(f"❌ Error: {response.text}")
