...
def init_websocket(socketio: SocketIO):
    ...
    def handle_connect(auth=None):
        ...
        user_id = auth.get('user_id', f'anonymous_{uuid.uuid4().hex[:8]}') if auth else f'anonymous_{uuid.uuid4().hex[:8]}'
        ...
        logging.info(f"Client connected: {user_id} ({user_type}) - Session: {session_id}")
    ...
    def handle_disconnect():
        ...
        with connection_lock:
            if session_id in active_connections:
                ...
                logging.info(f"Client disconnected: {connection_info.user_id} - Session: {session_id}")
    ...
    def handle_driver_location_update(data):
        ...
        try:
            ...
            socketio.emit('driver_location', asdict(location), room=f'driver_{location.driver_id}')
            
            # If on a route, also broadcast to route subscribers
            if location.route_id:
                socketio.emit('driver_location', asdict(location), room=f'route_{location.route_id}')
            
            emit('location_updated', {'status': 'success', 'timestamp': location.timestamp})
    ...
    def handle_route_status_update(data):
        ...
        try:
            ...
            socketio.emit('route_update', asdict(route_update), room=f'route_{route_update.route_id}')
            
            # Broadcast to all dispatchers and admins
            socketio.emit('fleet_update', {
                'type': 'route_status',
                'data': asdict(route_update)
            }, room='dispatchers')
            
            emit('route_updated', {'status': 'success', 'timestamp': route_update.timestamp})
