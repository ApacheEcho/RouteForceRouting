...
class EnhancedDashboardDemo:
    ...
    def demo_algorithm_comparison(self):
        ...
        sample_stores = [
            ...
            {
                "name": "Store H - Richmond",
                "lat": 37.8149,
                "lng": -122.3794,
                "priority": 2,
            },
            {
                "name": "Store I - Chinatown",
                "lat": 37.7949,
                "lng": -122.4070,
                "priority": 1,
            },
                ...
                for i, result in enumerate(results, 1):
                    if result["success"]:
                        ...
                        if result.get("initial_distance") and result.get(
                            ...
                            print(
                                f"     Distance: {initial_dist:.2f}km → {final_dist:.2f}km (saved {distance_saved:.2f}km)"
                            )
                        print()
                    else:
                        print(
                            f"❌ {result['algorithm']}: {result.get('error', 'Unknown error')}"
                        )
    ...
    def demo_performance_analytics(self):
        ...
        try:
            response = self.session.get(
                f"{self.base_url}/dashboard/api/performance/history"
            )
            if response.status_code == 200:
                data = response.json()
                    ...
                    for entry in data["history"]:
                        ...
                        for algo, metrics in entry.items():
                            if algo != "date" and isinstance(metrics, dict):
                                ...
                                print(
                                    f"     {algo}: {improvement}% improvement, {time_taken}s avg time"
                                )
    ...
    def demo_algorithm_details(self):
        ...
        for algorithm in algorithms:
            try:
                response = self.session.get(
                    f"{self.base_url}/dashboard/api/algorithm/details/{algorithm}"
                )
                if response.status_code == 200:
                    data = response.json()
            ...
            if response.status_code == 200:
                ...
                if "recent_activity" in data:
                    ...
                    for activity in data["recent_activity"][
                        ...
                        print(
                            f"   {timestamp.strftime('%H:%M:%S')} - {activity['algorithm']}: {activity['improvement']}% improvement ({activity['stores']} stores)"
                        )
