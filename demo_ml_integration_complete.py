...
def test_ml_api_direct():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"   Improvement: {genetic_data.get('metrics', {}).get('improvement_percent', 0):.2f}%")
        else:
        ...
        response = requests.post(f"{base_url}/routes/optimize/simulated_annealing", json=test_data)
        if response.status_code == 200:
            sa_data = response.json()
            print(f"   ✓ Simulated annealing optimization successful")
            print(f"   Improvement: {sa_data.get('metrics', {}).get('improvement_percent', 0):.2f}%")
        else:
            print(f"   ✗ Simulated annealing optimization failed: {response.status_code}")
...
def demonstrate_optimization_algorithms():
    ...
    test_data = {
        "stores": [
            {"id": "store_1", "name": "Manhattan Store", "lat": 40.7128, "lon": -74.0060, "priority": 1, "demand": 150},
            {"id": "store_2", "name": "Brooklyn Store", "lat": 40.6782, "lon": -73.9442, "priority": 2, "demand": 200},
            {"id": "store_3", "name": "Queens Store", "lat": 40.7282, "lon": -73.7949, "priority": 1, "demand": 100},
            {"id": "store_4", "name": "Bronx Store", "lat": 40.8448, "lon": -73.8648, "priority": 3, "demand": 120},
            {"id": "store_5", "name": "Staten Island Store", "lat": 40.5795, "lon": -74.1502, "priority": 2, "demand": 180},
            {"id": "store_6", "name": "Harlem Store", "lat": 40.8176, "lon": -73.9782, "priority": 1, "demand": 90},
            {"id": "store_7", "name": "LIC Store", "lat": 40.7505, "lon": -73.9370, "priority": 2, "demand": 140},
            {"id": "store_8", "name": "Williamsburg Store", "lat": 40.7081, "lon": -73.9571, "priority": 1, "demand": 160}
        ]
    }
    ...
    algorithms = [
        ...
        {"name": "Simulated Annealing", "endpoint": "routes/optimize/simulated_annealing"}
    ]
    ...
    for algorithm in algorithms:
        ...
        try:
            ...
            response = requests.post(f"{base_url}/{algorithm['endpoint']}", json=test_data)
    ...
    for result in results:
        if result['success']:
            print(f"{result['algorithm']:<20} | {result['improvement']:>6.1f}% | {result['processing_time']:>8.3f}s | {result['route_length']:>3d} stops")
        else:
...
def main():
    ...
    if success:
        ...
        if successful_results:
            ...
            print(f"✓ Best performing algorithm: {best_algorithm['algorithm']} ({best_algorithm['improvement']:.1f}% improvement)")
    else:
