...
def demo_multi_objective_complete():
    ...
    stores = [
        {"id": 1, "name": "Downtown Store", "lat": 40.7128, "lon": -74.0060, "priority": 1, "demand": 50, "service_time": 0.5},
        {"id": 2, "name": "Central Park Store", "lat": 40.7589, "lon": -73.9851, "priority": 2, "demand": 30, "service_time": 0.3},
        {"id": 3, "name": "Queens Store", "lat": 40.7282, "lon": -73.7949, "priority": 3, "demand": 40, "service_time": 0.4},
        {"id": 4, "name": "Brooklyn Store", "lat": 40.6782, "lon": -73.9442, "priority": 1, "demand": 60, "service_time": 0.6},
        {"id": 5, "name": "Midtown Store", "lat": 40.7505, "lon": -73.9934, "priority": 2, "demand": 25, "service_time": 0.25},
        {"id": 6, "name": "Upper West Store", "lat": 40.7831, "lon": -73.9712, "priority": 3, "demand": 35, "service_time": 0.35},
        {"id": 7, "name": "Financial District", "lat": 40.7061, "lon": -74.0087, "priority": 1, "demand": 45, "service_time": 0.45},
        {"id": 8, "name": "East Side Store", "lat": 40.7614, "lon": -73.9776, "priority": 2, "demand": 55, "service_time": 0.55},
        {"id": 9, "name": "Harlem Store", "lat": 40.8176, "lon": -73.9782, "priority": 3, "demand": 20, "service_time": 0.2},
        {"id": 10, "name": "Battery Park Store", "lat": 40.7033, "lon": -74.0170, "priority": 1, "demand": 65, "service_time": 0.65}
    ]
    ...
    for i, scenario in enumerate(scenarios):
        ...
        try:
            ...
            print(f"✓ Pareto front size: {metrics.get('pareto_front_size', 0)} solutions")
            ...
            for j, store in enumerate(best_route[:5]):  # Show first 5 stops
                print(f"  {j+1}. {store['name']} (Priority: {store.get('priority', 'N/A')}, Demand: {store.get('demand', 'N/A')})")
            if len(best_route) > 5:
    ...
    if successful_scenarios:
        print(f"✅ Successful optimizations: {len(successful_scenarios)}/{len(results)}")
        ...
        print(f"🏆 Best Pareto front: {best_scenario['pareto_front_size']} solutions ({best_scenario['scenario']})")
    ...
    print("The system can now handle complex routing scenarios with multiple competing objectives.")
