...
def demo_simulated_annealing_integration():
    ...
    stores = [
        {'name': 'Midtown Store', 'lat': 40.7589, 'lon': -73.9851, 'address': '123 Main St'},
        {'name': 'SoHo Store', 'lat': 40.7505, 'lon': -73.9934, 'address': '456 Broadway'},
        {'name': 'Upper East Side', 'lat': 40.7614, 'lon': -73.9776, 'address': '789 Park Ave'},
        {'name': 'Greenwich Village', 'lat': 40.7549, 'lon': -73.9840, 'address': '101 5th Ave'},
        {'name': 'Tribeca Store', 'lat': 40.7484, 'lon': -73.9857, 'address': '202 Broadway'},
        {'name': 'Chelsea Store', 'lat': 40.7580, 'lon': -73.9855, 'address': '303 Madison Ave'},
        {'name': 'Financial District', 'lat': 40.7505, 'lon': -73.9934, 'address': '404 Wall St'},
        {'name': 'Lower East Side', 'lat': 40.7614, 'lon': -73.9776, 'address': '505 5th Ave'},
        {'name': 'Chinatown Store', 'lat': 40.7155, 'lon': -73.9976, 'address': '606 Mott St'},
        {'name': 'Little Italy', 'lat': 40.7195, 'lon': -73.9965, 'address': '707 Mulberry St'},
    ]
    ...
    for algo_key, algo_name in algorithms:
        ...
        iterations = algorithm_metrics.get('total_iterations', 0) or algorithm_metrics.get('generations', 0)
    ...
    print("-" * 70)
    print(f"{'Algorithm':<20} | {'Time (s)':<8} | {'Improvement':<12} | {'Distance (km)':<12} | {'Iterations':<10}")
    print("-" * 70)
    for result in results:
        print(f"{result['name']:<20} | {result['time']:<8.3f} | {result['improvement']:<12.1f} | {result['distance']:<12.2f} | {result['iterations']:<10}")
    ...
    for config in sa_configs:
        ...
        print(f"      Improvement: {algorithm_metrics.get('improvement_percent', 0):.1f}%")
        print(f"      Final distance: {algorithm_metrics.get('final_distance', 0):.2f} km")
        print(f"      Iterations: {algorithm_metrics.get('total_iterations', 0)}")
        print(f"      Acceptance rate: {algorithm_metrics.get('acceptance_rate', 0):.2f}")
    ...
    print(f"   Route improvement: {algorithm_metrics.get('improvement_percent', 0):.1f}%")
