...
def get_render_services():
    ...
    if not api_key:
        print(
            "❌ RENDER_API_KEY not set. Export it or add to your environment (never commit real keys)."
        )
        return None
    ...
    try:
        ...
        if not services:
            print(
                "📋 No services found. You'll need to create services in your Render dashboard."
            )
        ...
        for i, service in enumerate(services, 1):
            ...
            print(
                f'   gh secret set RENDER_STAGING_SERVICE_ID --body="{service_id}"'
            )
            if len(services) > 1:
                print(
                    f'   gh secret set RENDER_PRODUCTION_SERVICE_ID --body="{service_id}"'
                )

        print("\n🚀 Next steps:")
        print("   1. Set the service IDs as GitHub secrets (commands above)")
        print(
            "   2. Test deployment: ./scripts/deploy-render.sh deploy --environment staging --dry-run"
        )
