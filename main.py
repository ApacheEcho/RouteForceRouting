...
def validate_file_content(file_path: str) -> bool:
    ...
    try:
        ...
        has_name = any(key.lower().replace(' ', '_') in name_fields for key in first_store.keys())
...
def generate():
    ...
    try:
        ...
        if not validate_file_content(file_path):
            os.remove(file_path)
            return jsonify({'error': 'Invalid file format or missing required columns (name field required)'}), 400
    ...
    try:
        if file_path.endswith('.csv'):
            ...
            try:
                ...
                raise NotImplementedError("Excel file support requires pandas and openpyxl packages")
        else:
...
def apply_proximity_sorting(route: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        if not all('latitude' in store and 'longitude' in store for store in route):
            logger.warning("Stores missing coordinate information for proximity sorting")
            return route
                ...
                if key != "chain":
                    ...
                    if key in ["max_route_stops"]:
                        try:
                            ...
                            invalid_rows.append({'row': idx, 'field': key, 'error': 'Must be an integer'})
                    if key in ["min_sales_threshold"]:
                        try:
                            float(value)
                        except ValueError:
                            invalid_rows.append({'row': idx, 'field': key, 'error': 'Must be a float'})
