...
class NightlyOptimizationRunner:
    ...
    def run_comprehensive_optimization(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not uga.config.use_numba:
                results["recommendations"].append(
                    "Install numba for 10-15% performance boost"
                )

        except Exception as e:
            print(f"  ❌ Ultra Genetic Algorithm: Error - {e}")
            results["components"]["ultra_genetic_algorithm"] = {
                "status": "error",
                "error": str(e),
            }
        ...
        try:
            ...
            results["components"]["api_optimizer"] = {
                "status": "error",
                "error": str(e),
            }
            results["overall_status"] = "degraded"
        ...
        try:
            ...
            print(
                "  ✅ Performance Dashboard: Ready (SocketIO needed for WebSocket)"
            )
            results["recommendations"].append(
                "Configure SocketIO for real-time dashboard updates"
            )

        except Exception as e:
            print(f"  ❌ Performance Dashboard: Error - {e}")
            results["components"]["performance_dashboard"] = {
                "status": "error",
                "error": str(e),
            }
            results["overall_status"] = "degraded"
        ...
        try:
            ...
            print(
                "  ✅ Optimized Redis Cache: Ready (Redis connection needed)"
            )
            results["recommendations"].append(
                "Configure Redis connection for full cache optimization"
            )

        except Exception as e:
            print(f"  ❌ Optimized Redis Cache: Error - {e}")
            results["components"]["optimized_cache"] = {
                "status": "error",
                "error": str(e),
            }
            results["overall_status"] = "degraded"
        ...
        try:
            ...
            print(
                "  ✅ Optimized Connection Pool: Ready (DB connection needed)"
            )
            results["recommendations"].append(
                "Configure database URL for full connection pool optimization"
            )

        except Exception as e:
            print(f"  ❌ Optimized Connection Pool: Error - {e}")
            results["components"]["optimized_connection_pool"] = {
                "status": "error",
                "error": str(e),
            }
            results["overall_status"] = "degraded"
        ...
        for comp_data in results["components"].values():
            if (
                ...
                and "performance_score" in comp_data
            ):
        ...
        if overall_score < 90:
            results["recommendations"].append(
                "Consider installing optional dependencies (numba, cython) for performance boost"
            )
    ...
    def generate_report(self, results: Dict[str, Any]):
        ...
        print(
            f'Overall Performance Score: {results["performance_scores"]["overall"]:.1f}/100'
        )
        print("")
        ...
        for component, data in results["components"].items():
            ...
            print(
                f'  {component.replace("_", " ").title()}: {status}{score_str}'
            )
