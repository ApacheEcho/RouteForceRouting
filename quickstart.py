...
def setup_database():
    ...
    # Create migration
    run_command("flask db migrate -m 'Initial migration'", "Creating migration")
...
def run_tests():
    ...
    for cmd in test_commands:
        ...
        print("❌ No test runner worked. Make sure tests are set up correctly.")
...
def install_dependencies():
    ...
    if os.path.exists('requirements.txt'):
        run_command("pip3 install -r requirements.txt", "Installing from requirements.txt")
    else:
    ...
    if os.path.exists('requirements-dev.txt'):
        run_command("pip3 install -r requirements-dev.txt", "Installing development dependencies")
...
def format_code():
    ...
    for cmd, description in formatters:
        try:
            ...
            print(f"ℹ️  {description} not available (install with: pip install {cmd.split()[0]})")
