...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="RouteForce Auto-Commit Service CLI"
    )
    ...
    status_parser = subparsers.add_parser(
        "status", help="Check service status"
    )

    # Start command
    start_parser = subparsers.add_parser(
        "start", help="Start the auto-commit service"
    )
    ...
    stop_parser = subparsers.add_parser(
        "stop", help="Stop the auto-commit service"
    )

    # Force commit command
    commit_parser = subparsers.add_parser(
        "commit", help="Force immediate commit"
    )

    # Test command
    test_parser = subparsers.add_parser(
        "test", help="Test auto-commit functionality"
    )
    ...
    if args.command == "status":
        print(
            f"Auto-commit service status: {'running' if service.is_running else 'stopped'}"
        )
        ...
        service.start()
        print(
            f"Auto-commit service started with {args.interval}-minute intervals"
        )
