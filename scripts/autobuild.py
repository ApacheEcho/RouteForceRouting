    ...
    if tasks_to_add:
        with open(TODO_PATH, "a") as f:
            for task in tasks_to_add:
                f.write(f"{task}\n")
        print(
            f"🧠 Copilot suggested {len(tasks_to_add)} new task(s), added to todo list."
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Autonomous Copilot Autobuild Script"
    )
    parser.add_argument(
        "--autoloop", action="store_true", help="Enable autoloop mode"
    )
    parser.add_argument(
        "--autosave", action="store_true", help="Enable autosave mode"
    )
    parser.add_argument(
        "--cleanup",
        action="store_true",
        help="Cleanup temp files after each task",
    )
    parser.add_argument(
        "--notify", action="store_true", help="Notify on task completion"
    )
