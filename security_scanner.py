...
def run_codeql_query(query_file: str, query_name: str) -> dict:
    ...
    try:
        ...
        actual_results = [
            ...
            if line.strip() != "| call | col1 |" and "------" not in line
        ]
...
def generate_security_report(results: list) -> None:
    ...
    for result in results:
        ...
        print(
            f"{status_icon} {result['query']}: {result['vulnerabilities_found']} vulnerabilities"
        )
...
def save_report_to_file(
    results: list, timestamp: str, total_vulnerabilities: int
) -> None:
        ...
        "summary": {
            ...
            "path_traversal": next(
                (
                    r["vulnerabilities_found"]
                    for r in results
                    if "traversal" in r["query"]
                ),
                0,
            ),
            "command_injection": next(
                (
                    r["vulnerabilities_found"]
                    for r in results
                    if "Command" in r["query"]
                ),
                0,
            ),
            "xss": next(
...
def main():
    ...
    if not os.path.exists("codeql-database"):
        ...
        print(
            "   Run: codeql database create --language=python --source-root=. codeql-database"
        )
        return
    ...
    for query_file, query_name in queries:
        if os.path.exists(f"codeql-custom-queries-python/{query_file}"):
            ...
            if result["status"] == "success":
                if result["vulnerabilities_found"] == 0:
                    print_success(
                        f"No {query_name.lower()} vulnerabilities found"
                    )
                else:
                    print_warning(
                        f"Found {result['vulnerabilities_found']} potential {query_name.lower()} issues"
                    )
            else:
                print_warning(
                    f"Error running {query_name}: {result.get('error', 'Unknown error')}"
                )
        else:
