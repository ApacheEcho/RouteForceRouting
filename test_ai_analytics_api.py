...
def run_comprehensive_test():
    ...
    result = test_api_endpoint("/insights/route/test_route_ai?distance=35.2&duration=95&stops=6&fuel_used=4.1")
    ...
    if result["success"]:
        ...
        print(f"   ✅ Success: Predicted duration {prediction['predicted_duration_minutes']} min")
    ...
    result = test_api_endpoint("/recommendations/smart?vehicle_count=3&time_window=afternoon&priority=speed")
    tests.append(("Smart Recommendations", result))
    
    if result["success"]:
        recommendations = result["data"]["recommendations"]
        print(f"   ✅ Success: {len(recommendations['smart_suggestions'])} smart suggestions")
        print(f"   🎯 Confidence: {recommendations['confidence']}")
        print(f"   📝 Context: {recommendations['context']['time_window']} / {recommendations['context']['priority']}")
    else:
        print(f"   ❌ Failed: {result['data']}")
    ...
    batch_data = {
        "routes": [
            {"id": "batch_01", "distance": 25.0, "duration": 80, "fuel_used": 3.2, "stops": ["A", "B", "C"]},
            {"id": "batch_02", "distance": 45.5, "duration": 150, "fuel_used": 6.1, "stops": ["D", "E", "F", "G"]},
            {"id": "batch_03", "distance": 15.2, "duration": 65, "fuel_used": 2.1, "stops": ["H", "I"]}
        ]
    }
