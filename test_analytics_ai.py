...
def test_analytics_engine():
    ...
    # Patch: Run all test logic inside Flask app context
    ...
    with app.app_context():
        ...
        assert analytics is not None, "Failed to create analytics engine"
        ...
        assert (
            hasattr(analytics, "historical_data") and len(analytics.historical_data) > 0
        ), "Sample route data not added"
        print("✅ Sample route data added")

        # Test generating insights
        insight = analytics.analyze_route_efficiency("test_001", sample_route)
        assert insight is not None, "No insight generated"
        ...
        assert fleet_insights is not None and fleet_insights["total_routes"] > 0, (
            "No fleet insights generated"
        )
    ...
    assert len(analytics.routes) >= 21, "Not all sample routes added for ML training"
    print("✅ Added 20 sample routes for ML training")

    # Test trends after adding data
    trends = analytics.detect_performance_trends()
    assert trends is not None and len(trends) > 0, "No performance trends detected"
