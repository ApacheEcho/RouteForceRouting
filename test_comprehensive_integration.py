        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if prediction_result.get("success"):
                    ...
                    logger.info(
                        f"  Predicted Duration: {prediction_result['prediction']['predicted_duration']:.1f} min"
                    )
                    logger.info(
                        f"  Predicted Fuel Cost: ${prediction_result['prediction']['predicted_fuel_cost']:.2f}"
                    )
                    return True
                else:
    ...
    def test_enterprise_dashboard(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"✗ Dashboard overview request failed: {response.status_code}"
                )
                return False
            ...
            if response.status_code == 200:
                ...
                if routes_data.get("success"):
                    ...
                    if routes:
                        route = routes[0]
                        logger.info(
                            f"  Sample Route: {route.get('route_id')} - {route.get('status')}"
                        )
                    return True
                else:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if demo_data.get("success"):
                    ...
                    logger.info(
                        f"  Routes Generated: {demo_data.get('routes_generated', 0)}"
                    )
                    return True
                else:
    ...
    def test_route_optimization(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"✗ Route optimization request failed: {response.status_code}"
                )
                return False
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if trends_data.get("success"):
                    ...
                    for trend in trends:
                        logger.info(
                            f"  {trend.get('metric_name', 'N/A')}: {trend.get('trend_direction', 'N/A')}"
                        )
                    return True
                else:
                ...
                logger.error(
                    f"✗ Performance trends request failed: {response.status_code}"
                )
                return False
