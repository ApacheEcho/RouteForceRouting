...
def test_mobile_auth():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"Session token generated: {result.get('session_token', 'N/A')[:20]}...")
            print(f"Expires at: {result.get('expires_at', 'N/A')}")
            print(f"Device capabilities: {len(result.get('device_capabilities', {}))} features")
...
def test_mobile_route_optimization():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"Route generated with {len(result.get('route', {}).get('stops', []))} stops")
...
def test_driver_location_updates():
    ...
    for i, location in enumerate(locations):
        ...
        try:
            ...
            if response.status_code == 200:
                success_count += 1
                print(f"  📍 Location {i+1}: ✅ Updated ({location['lat']:.4f}, {location['lng']:.4f})")
            else:
    ...
    for status in statuses:
        test_data = {
            ...
            "metadata": {
                ...
                "route_id": "route_123" if status in ['busy', 'en_route', 'delivering'] else None
            }
        }
