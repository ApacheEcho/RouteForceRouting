...
def test_simulated_annealing_basic():
    ...
    try:
        ...
        assert False, f"Error during optimization: {str(e)}"
...
def test_simulated_annealing_performance():
    ...
    assert len(results) == len(test_sizes), (
        f"Some performance tests failed: {len(results)}/{len(test_sizes)} completed."
    )
...
def test_simulated_annealing_comparison():
    ...
    assert len(results) == len(configurations), (
        f"Some configuration tests failed: {len(results)}/{len(configurations)} completed."
    )
...
def main():
    ...
    # Run tests (pytest will discover and run these, but for direct script execution, call them here)
    test_simulated_annealing_basic()
    test_simulated_annealing_custom_config()
    test_simulated_annealing_performance()
    test_simulated_annealing_comparison()

    # Print summary
    print("\n" + "=" * 70)
    print("🎯 Test Results Summary")
    print("=" * 70)
    print("All Simulated Annealing tests executed. For pass/fail, see pytest output.")
