...
def test_simulated_annealing_basic():
    ...
    try:
        ...
        for i, stop in enumerate(optimized_route):
            print(f"  {i+1}. {stop.get('name', 'Unknown')} - {stop.get('address', 'No address')}")
...
def test_simulated_annealing_performance():
    ...
    for size in test_sizes:
        ...
        try:
            ...
            print(f"  ✅ {size} stores: {processing_time:.2f}s, {result['iterations']} iterations")
    ...
    for result in results:
        print(f"{result['stores']:6} | {result['processing_time']:8.2f} | {result['iterations']:10} | {result['best_fitness']:12.2f}")
...
def test_simulated_annealing_comparison():
    ...
    print("-" * 70)
    for result in results:
        print(f"{result['name']:18} | {result['processing_time']:8.2f} | {result['iterations']:10} | {result['best_fitness']:12.2f} | {result['final_temperature']:10.4f}")
...
def main():
    ...
    test_results.append(("Custom Configuration", test_simulated_annealing_custom_config()))
    test_results.append(("Performance Testing", test_simulated_annealing_performance()))
    test_results.append(("Configuration Comparison", test_simulated_annealing_comparison()))
