...
class TestAutoCommitService(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.service = AutoCommitService(
            repo_path=self.test_dir, interval_minutes=1
        )
    ...
    def test_ensure_wip_branch_switches_to_existing(self, mock_run):
        ...
        mock_run.assert_any_call(
            ["git", "checkout", "auto-wip"], cwd=self.test_dir, check=True
        )
    ...
    def test_commit_and_push_success(self, mock_run):
        ...
        for expected_call in expected_calls:
            mock_run.assert_any_call(
                expected_call, cwd=self.test_dir, check=True
            )
