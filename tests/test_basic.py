...
class TestRouteOptimization:
    ...
    def test_route_optimization_basic(self, sample_locations):
        ...
        def nearest_neighbor_route(locations):
            ...
            while remaining:
                ...
                nearest = min(
                    remaining,
                    key=lambda loc: (
                        (current["lat"] - loc["lat"]) ** 2
                        + (current["lng"] - loc["lng"]) ** 2
                    )
                    ** 0.5,
                )
        ...
        assert (
            optimized_route[0] == sample_locations[0]
        )  # Starts at first location
...
class TestMonitoring:
    ...
    def test_error_tracking(self):
        ...
        error_info = track_error(
            "ValidationError", "Invalid input", {"field": "location"}
        )
    ...
    def test_end_to_end_routing(self, sample_locations):
        ...
        def full_routing_workflow(locations):
            ...
            while remaining:
                ...
                nearest = min(
                    remaining,
                    key=lambda x: distances.get(
                        f"{current}-{x}", float("inf")
                    ),
                )
                route.append(nearest)
                remaining.remove(nearest)
            ...
            return {
                ...
                "route": route,
                "total_distance": sum(
                    distances.get(f"{route[i]}-{route[i+1]}", 0)
